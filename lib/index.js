...
const Config = z.object({ sensitivePaths: z.boolean().default(true) });
/**
...
*/
function isKeyMaterialPath(path) {
...
function isSensitivePath(path) {
	const segments = path.replaceAll("\\", "/").split("/").filter((segment) => segment.length > 0);
	if (segments.length === 0) return false;
	for (const segment of segments) if (segment === ".git" || segment === ".ssh" || segment.startsWith(".env")) return true;
	return isKeyMaterialBasename(segments[segments.length - 1]);
}
/**
...
function classifySensitivePath(path) {
	...
	if (basename !== void 0 && isKeyMaterialBasename(basename)) return basename.endsWith(".pem") ? "certificate" : "key-material";
...
function sensitiveTarget(exec) {
	if (exec.name === "write" || exec.name === "edit" || exec.name === "str_replace_editor") {
		...
		const candidate = typeof args === "object" && args !== null ? args[field] : void 0;
		return typeof candidate === "string" && isSensitivePath(candidate) ? {
			target: candidate,
			category: classifySensitivePath(candidate)
		} : void 0;
	}
	if (exec.name === "read") {
		const args = exec.arguments;
		const candidate = typeof args === "object" && args !== null ? args.file_path : void 0;
	...
	if (exec.name === "bash") {
		const args = exec.arguments;
		const command = typeof args === "object" && args !== null ? args.command : void 0;
		if (typeof command !== "string") return void 0;
		const fragment = command.match(SENSITIVE_COMMAND_SCAN)?.[0];
		return fragment === void 0 ? void 0 : {
			target: fragment,
			category: classifySensitivePath(fragment)
		};
	}
}
/**
...
//#endregion
export { Config, SENSITIVE_PATH_GLOBS, apply, inject, isKeyMaterialPath, isSensitivePath, name };
